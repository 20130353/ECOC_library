...
def ECOC_Process(train_data,train_label,test_data,test_label,ECOC_name,**param):
    ...
    learners = ['','base_estimator=RandomForestClassifier']
    ...
    if ECOC_name.find('DC_ECOC') >= 0:
        ...
        if 'base_M' in param:
            E = eval('Self_Adaption_ECOC(' + learners[0] + ')')
...
def get_base_M(path,ecoc,dataname):
    ...
    dc_option = ['F1', 'F2', 'F3', 'N2', 'N3', 'N4', 'L3', 'Cluster','OVO_ECOC', 'OVA_ECOC', 'Dense_random', 'Sparse_random', 'D_ECOC']
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='E:/workspace1/ECOCDemo/Microarray_res/SVC/SAT_Other/SAT_Opther_log.txt', level=logging.DEBUG, format=LOG_FORMAT)
    ...
    ECOC_conbination_123 = ['Self_Adaption_ECOC F1 F1 DC', 'Self_Adaption_ECOC F2 F2 DC' \
        , 'Self_Adaption_ECOC N3 N3 DC', 'Self_Adaption_ECOC F1 F2 DC', 'Self_Adaption_ECOC F1 N3 DC'
        , 'Self_Adaption_ECOC F2 N3 DC','Self_Adaption_ECOC F1 F2 N3 DC']

    ECOC_conbination_other = [ 'Self_Adaption_ECOC other OVO_ECOC OVA_ECOC','Self_Adaption_ECOC other OVO_ECOC Dense_random'\
                              ,'Self_Adaption_ECOC other OVO_ECOC Sparse_random','Self_Adaption_ECOC other OVO_ECOC D_ECOC'\
                              ,'Self_Adaption_ECOC other OVA_ECOC Dense_random','Self_Adaption_ECOC other OVA_ECOC Sparse_random'\
                              ,'Self_Adaption_ECOC other OVA_ECOC D_ECOC','Self_Adaption_ECOC other Dense_random Sparse_random'\
                              ,'Self_Adaption_ECOC other Dense_random D_ECOC','Self_Adaption_ECOC other OVO_ECOC OVA_ECOC Dense_random'\
                              ,'Self_Adaption_ECOC other OVO_ECOC OVA_ECOC Sparse_random','Self_Adaption_ECOC other OVO_ECOC OVA_ECOC D_ECOC'\
                              ,'Self_Adaption_ECOC other OVA_ECOC Sparse_random,Dense_random','Self_Adaption_ECOC other OVA_ECOC Sparse_random,D_ECOC'\
                              ,'Self_Adaption_ECOC other Sparse_random,Dense_random,D_ECOC']
    ...
    matrix_folder_path = 'E:/workspace1/ECOCDemo/Microarray_res/SVC/DC_matrix/'
    other_matrix_folder_path = 'E:/workspace1/ECOCDemo/Microarray_res/SVC/other_ECOC_matrix/'
    res_folder_path = 'E:/workspace1/ECOCDemo/Microarray_res/SVC/SAT_Other/all_Other_combination/'

    selected_dataname = microarray_dataname
    selected_ecoc_name = ECOC_conbination_other
    ...
    for k in range(len(selected_fs_name)):
        ...
        for i in range(len(selected_dataname)):
            ...
            for j in range(len(selected_ecoc_name)):
                ...
                param = {}
                if selected_ecoc_name[j].find('Self_Adaption_ECOC') >=0 and \
                    selected_ecoc_name[j].find('other') >= 0:
                    final_folder_matrix_path = other_matrix_folder_path + selected_fs_name[k] + '/'
                    param['base_M'] = get_base_M(final_folder_matrix_path,selected_ecoc_name[j],selected_dataname[i])

                elif selected_ecoc_name[j].find('Self_Adaption_ECOC') >=0:
